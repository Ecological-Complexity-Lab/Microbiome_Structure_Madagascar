...
betaNTI_final_mandena <- betaNTI_three_villages %>% 
  ...
  mutate(rescale_betaNTI = ifelse(rescale_betaNTI<0.8,0,rescale_betaNTI)) # rescale the values

# igraph
data_for_graph <- betaNTI_final_mandena %>% select(-mean_betaNTI) %>% filter(rescale_betaNTI > 0)
