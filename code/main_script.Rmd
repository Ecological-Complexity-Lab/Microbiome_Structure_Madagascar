...
total_reads_groups_grid <- data_asv_tax %>% distinct(host_ID,asv_core,grid) %>%  group_by(asv_core, grid) %>% summarise(n_total = sum(total_reads))
g <- data_asv_tax %>% 
...
# total_reads_groups <- data_asv_tax %>% distinct(host_ID, asv_core) %>% group_by(asv_core) %>% summarise(n_total = sum(total_reads))
...
#   
#   ggplot(aes(fill=Family, x=asv_core, y=n_p)) +
#   geom_bar(position="fill", stat="identity") +
#   #facet_wrap(~asv_core) +
#   theme_bw() +
#   theme(axis.text = element_text(size = 10, color = 'black', angle = 90, vjust = 0.5, hjust=1), title = element_text(size = 14)) +
