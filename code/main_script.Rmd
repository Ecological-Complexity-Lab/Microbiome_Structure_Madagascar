...
for (v in core_names) {
  ...
  sum_reads <- data_asv_group %>% distinct(host_ID,total_reads) %>% sum(sum_reads$total_reads)
  rel_abu <- data_asv_group %>% 
    mutate(reads=reads*total_reads) %>%
    group_by(asv_ID) %>% 
    summarise(reads_sum = sum(reads)) %>% 
    mutate(rel_reads = reads_sum/sum_reads)
  
  asv_distance2 <- asv_distance[rownames(asv_distance)%in%asv_pool$asv_ID , colnames(asv_distance)%in%asv_pool$asv_ID]
  assembly <- iCAMP::qpen(data_assembly, pd = asv_distance2, taxo.metric="jaccard", rand=100, sig.bNTI = 2, sig.rc = 0.95, ab.weight=F, nworker = 4)
    raupc <- iCAMP::RC.pc(data_assembly, taxo.metric="bray", rand=100, weighted=F,  nworker = 4, output.bray = T, meta.ab = rel_abu$rel_reads)
