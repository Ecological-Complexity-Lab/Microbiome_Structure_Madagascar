...
betaNTI <- fun_calc_betaNTI(data_betaNTI, asv_distance, asv_pool) %>% mutate(village = v)
...
raupc_three_villages <- rbind(raupc_three_villages, raupc2)
}
```
# Modularity analysis
...
for (j in 1:3) {
  ...
  grid_modules_mat <- final_data %>% 
    filter(village == village_names[j]) %>% 
    select(grid1, grid2, module_similarity) %>% 
    spread(grid1, module_similarity)
  print(psych::pairs.panels(final_data %>% filter(village == village_names[j]) %>%  select(-village,-grid1,-grid2, -module_similarity), ellipses = F, lm = T))
  cat('\n','\n')
}
```
