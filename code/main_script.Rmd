...
best_tree <- readRDS(file = "../results/phylo_tree_rra0.001_p0.01_microgale.rds")
...
for (v in core_names) {
  ...
  # raup-crick
  data_raupc <- data_asv_group %>% 
    distinct(host_ID, asv_ID, reads) %>% 
    spread(asv_ID, reads, fill = 0) %>% 
    column_to_rownames("host_ID") %>% 
    as.matrix()
  raupc <- iCAMP::RC.pc(data_raupc, taxo.metric="bray", rand=100, weighted=TRUE, dirichlet=TRUE, nworker = 4)
  ...
  assembly <- iCAMP::qpen(data_raupc, pd = asv_distance, taxo.metric="bray", rand=100, sig.bNTI = 2, sig.rc = 0.95, ab.weight=TRUE, nworker = 4)
}
```
