...
# how many Genus?
data_tax %>% group_by(asv_core) %>% summarise(n=n_distinct(Genus))
...
#plotting
# library(ggfortify)
# autoplot(tax_pca, data = data_tax_meta2, colour = 'asv_core', scale=0,
...
tax_scores %>% 
  ...
  annotate("label", x = (loading_top$PC1*6), y = (loading_top$PC2*6),
     label = loading_top$Genus, size=3.5, label.size=0.15, label.padding=unit(0.1, "lines")) +
  theme_bw() +
  scale_fill_manual(values=group.colors) +
...
# Perform PERMANOVA
permanova_result <- adonis2(distance_matrix ~ grid+season+mass+sex+age_dental+elevation.obs, data = hosts, permutations = 999)
...
nmds_plot <- cbind(hosts, NMDS1, NMDS2)
