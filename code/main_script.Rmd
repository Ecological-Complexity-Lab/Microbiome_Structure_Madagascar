...
assembly_final %<>% mutate(process = case_when(betaNTI>1.96 ~ "Heterogeneous Selection",
                                               betaNTI<(-1.96) ~ "Homogeneous Selection",
                                               (betaNTI<1.96 & betaNTI>(-1.96) & raupc>0.95) ~ "Dispersal Limitation",
                                               (betaNTI<1.96 & betaNTI>(-1.96 ) & raupc<(-0.95)) ~ "Homogenizing Dispersal",
                                               .default = "Drift"))
...
p1 <- nmi_diff_core_three_villages %>% 
  ...
  scale_size_continuous( breaks = c(1,10,40,60)) +
  scale_shape_manual(values = c(1, 16), guide="none") +
  scale_y_continuous(limits = c(0, 0.35)) +
  scale_x_continuous(limits = c(0, 0.2)) +
  theme_bw() +
  theme(axis.text = element_text(size = 12, color = 'black'), title = element_text(size = 15), panel.grid = element_blank(), panel.border = element_rect(color = "black"), legend.title = element_text(size = 12)) +
...
grid_distance_three_villages <- grid_distance_three_villages[c("Andatsakala", "Mandena", "Sarahandrano")]
