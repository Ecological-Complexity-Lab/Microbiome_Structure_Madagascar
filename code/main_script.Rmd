...
loading_top <- loadings %>% 
  slice_max(n = 10, order_by = PC1_abs) %>% 
  bind_rows(loadings %>% slice_max(n = 10, order_by = PC2_abs)) %>% 
  ungroup()
...
tax_scores %>% 
  ...
  geom_point() +
  geom_segment(data = loading_top, aes(x = 0, y = 0, xend = PC1*5, yend = PC2*5), 
