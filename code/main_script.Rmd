...
total_reads_groups_grid <- data_asv_tax %>% group_by(asv_core, grid) %>% summarise(n_total = sum(total_reads))
g <- data_asv_tax %>% 
...
total_reads_groups <- data_asv_tax %>% group_by(asv_core) %>% summarise(n_total = sum(total_reads))
