...
knitr::opts_chunk$set(results = "asis", message=FALSE, warning=FALSE, cache=TRUE, eval = TRUE, dev = c('pdf','png'), out.width = '100%', out.height='100%')
```
...
g <- final_network_properties %>% 
  ...
  theme(axis.text = element_text(size = 9, color = 'black', angle = 90, vjust = 0.5, hjust=1), title = element_text(size = 14), 
        ...
        panel.grid = element_blank(), panel.background = element_rect(colour = "black"), aspect.ratio = 1.8) +
...
rownames_to_column("var") %>% 
 ...
 g <- anova_model_all_plot %>% 
  ...
  theme(axis.text = element_text(size = 10, color = 'black'), title = element_text(size = 14), 
         #legend.position = "bottom",
        panel.grid = element_blank(), panel.border = element_rect(color = "black"), aspect.ratio = 0.6) +
  scale_color_manual(values=group.colors) +
