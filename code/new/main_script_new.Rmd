...
unifrac_result <- adonis2(distance_matrix ~ grid+season+sex, data = hosts, permutations = 999)
...
grid_sum_mat <- grid_sum %>% 
  unite(col="grid_village", grid_name:village, remove = TRUE) %>%
...
vegetation_pca <- as.data.frame(pca_veg$x) %>% 
  ...
  rownames(vegetation_pca) <- vegetation_pca$grid_village
...
# loop for three villages
library(psych)
print(psych::pairs.panels(grids_similarity_attr %>% filter(village == vil) %>%  select(-village,-grid1,-grid2), ellipses = F, lm = F))
cat('\n','\n')
```
...
for (i in 1:3) {
  ...
  vegetation_pca %<>% filter(grid_village %in% grid_names)
  modules_similarity2 <- modules_similarity2[rownames(vegetation_pca),]
  
  # db-RDA
  rda_result <- dbrda(modules_similarity2 ~ village + PC1 + PC2, data=vegetation_pca, distance="bray", na.action = na.omit)
  rda_result
  anova(rda_result)
  anova(rda_result, by = "margin", permutations = 999)
  coef(rda_result)
  sppscores(rda_result) <- modules_similarity2
  plot(rda_result)
}
```
