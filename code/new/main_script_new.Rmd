...
# correlations between variables
print(psych::pairs.panels(grids_similarity_attr %>%  select(-grid_village), ellipses = F, lm = F))
cat('\n','\n')
```
...
for (i in 1:3) {
  ...
  # db-RDA
  rda_result <- dbrda(modules_similarity2 ~ veg_PC1+veg_PC2+dist_to_village+elevation, data=grids_similarity_attr2, distance="bray", na.action = na.omit)
  rda_result
  anova(rda_result)
