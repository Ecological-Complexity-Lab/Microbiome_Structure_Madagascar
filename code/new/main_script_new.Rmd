---
title: "Distinct drivers of host-microbe network structures for core and rare microbes along a land use change gradient"
subtitle: "Analysis and results"
author: "Matan Markfeld"
date: "Last edit: 2024-09-04"
output: 
  html_document:
    toc: true
    toc_float: true
...
core_th <- c(0.02, 0.2)
data_asv_p <- data_asv %>% 
  mutate(asv_core = case_when(host_p<core_th[1] ~ "Rare",
...
for (v in core_names) {
  ...
  raupc <- as.matrix(raupcrick(data_assembly, nsimul = 500))
  assembly <- iCAMP::qpen(data_assembly, pd = asv_distance2, RC=raupc, rand=500, sig.bNTI = 2, sig.rc = 0.95, ab.weight=F, nworker = 4)
...
cat('## Modules','\n')
  modules_three_groups[[1]]+modules_three_groups[[2]]+modules_three_groups[[3]] + plot_layout(nrow=1,ncol=3,guides='collect',axes = "collect_y",axis_titles = "collect") &
  theme(legend.position='bottom')
  cat('\n','\n')
```
...
```{r, modules_exploration, eval=FALSE, include=FALSE}
# plotting
...
```{r, assembly_groups, eval=FALSE, include=FALSE}
# checking differences between groups

# Number of incidents across the four processes
assembly_summary_group <- assembly_summary %>% 
  group_by(asv_core, process) %>% 
  summarise(n = sum(n))
...
# Define all pairs of core groups
core_pairs <- list(c("Core", "Non-core"), c("Core", "Rare"), c("Non-core", "Rare"))
# Define all processes
process_names <- unique(assembly_summary_group$process)

# Initialize a list to store all p-values for multiple testing correction
all_p_values <- c()

# Loop through each process group
for (p in process_names) {
  # Loop through each pair of core groups for pairwise comparisons
  for (pair in core_pairs) {
    
    # getting the number incidents
    successes <- assembly_summary_group %>% 
      filter(asv_core %in% pair & process == p) %>% 
      left_join(assembly_summary_total, by="asv_core")
    
    # Perform the two-sample proportion test
    test_result <- prop.test(successes$n, successes$n_total)
    
    # Store the p-value
    all_p_values <- c(all_p_values, test_result$p.value)
    
    # Add the result to the dataframe (without correction yet)
    results_df <- rbind(results_df, data.frame(
      Process = p,
      Comparison = paste(pair[1], "vs", pair[2]),
      prop = test_result$estimate[1] - test_result$estimate[2],
      chi = test_result$statistic,
...
# Apply Bonferroni correction for multiple testing across all tests
adjusted_p_values <- p.adjust(all_p_values, method = "bonferroni")

# Assign the adjusted p-values back to the dataframe
results_df$p_value_adjusted <- adjusted_p_values
...
```{r, nmi, eval=FALSE, include=FALSE}
cat('## Observed NMI','\n')
...
for (i in 1:3) {
  ...
  rda_result <- rda(modules_similarity2_hell ~ veg_PC1+veg_PC2+dist_to_village+distance+elevation+Condition(n), data=grids_similarity_attr2, na.action = na.omit)
...
# significance test for the whole model
  ...
  rownames(rda_var) <- c("Veg_1","Veg_2","Dist-to-Village","Elevation", "Distance")
...
g <- rda_samples %>% 
  ...
  theme_bw() +
  coord_cartesian(
    xlim = range(rda_samples$RDA1) * c(1.4, 1.3), 
...
rda_figs[[1]] + rda_figs[[2]] + rda_figs[[3]] + plot_layout(ncol=3, nrow=1) & theme(legend.position='none')
cat('\n','\n')
```
...
```{r, mantel_test, eval=FALSE, include=FALSE}
...
```{r, eval=FALSE, include=FALSE}
# mantel test for vegetation attributes by grid-season

# vegetation similarity
habitat.raw <- read_csv("../data/data_raw/data_small_mammals/Trap_Plots.csv") %>% filter(village == vil)
grid_attributes <- fun_grid_attributes(habitat.raw)

mantel_result2_three_groups <- NULL
final_data2_three_groups <- NULL

# loop for three asv groups
for (i in 1:3) {
  
  modules_similarity <- modules_similarity2_three_groups[[i]]
  grid_names <- rownames(grid_attributes)
  # filtering the modules matrix to the existing grids
  modules_similarity <- modules_similarity[grid_names, grid_names]
  
  
  
  # calculating mantel test
  mantel_result <- vegan::mantel(modules_similarity, grid_attributes)
  mantel_result_df <- data_frame(r = round(mantel_result$statistic,2),
                                 pvalue = round(1-mantel_result$signif,4),
                                 asv_core = core_names[i])
  mantel_result2_three_groups <- rbind(mantel_result2_three_groups, mantel_result_df)
  ...
  final_data2_three_groups <- rbind(final_data2_three_groups, final_data)
