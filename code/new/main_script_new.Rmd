...
for (i in 1:3) {
  ...
  rda_result <- rda(modules_similarity2 ~ veg_PC1+veg_PC2+dist_to_village+elevation, data=grids_similarity_attr2, na.action = na.omit)
  rda_result
  
  # variation explained
  constrained_eig <- rda_result$CCA$eig/rda_result$tot.chi*100
...
R2.obs <- RsquareAdj (rda_result)$r.squared
R2.obs

# significance test
  anova(rda_result)
  rda_anova <- anova(rda_result, by = "margin", permutations = 999)
  rda_anova$`Pr(>F)` <- p.adjust (rda_anova$`Pr(>F)`, method = 'holm')
  rda_anova
  
  #coef(rda_result)
  sppscores(rda_result) <- modules_similarity2
  score_rda <- scores(rda_result)$species
  plot(rda_result)
}
```
