...
core_names <- unique(data_asv_final$asv_core)
...
most_abu_family <- data_asv_tax %>%
  ...
  most_abu_family_p <- data_asv_tax %>% left_join(modules_table_three_groups %>% mutate(module = factor(ifelse(host_group==1,1,0))) %>%
  distinct(host_ID,host_group,asv_core, module), by=c("host_ID","asv_core" )) %>% 
  mutate(reads_a = reads*total_reads) %>%
...
# plotting
g1 <- nmds_plot %>% filter(host_ID!=3357) %>% 
  ggplot( aes(MDS1, MDS2, color=module, shape=village)) +
...
# plotting
g2 <- nmds_plot2 %>% 
  ggplot( aes(MDS1, MDS2, color=module, shape=season)) +
