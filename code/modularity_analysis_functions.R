...
# function for calculating similarity in modules between grids-seasons
...
fun_grid_attributes <- function(dat) {
  ...
  habitat <- dat %>% 
    ...
    select(grid_name, season, plot, square, dead_logs, tree_dbh, tree_height, #omiting crown distance
  ...
  ## make a trees df
  trees <- habitat %>% 
    select(grid_name, season, pitfall, plot, square, tree_height, tree_dbh) %>% 
    group_by(grid_name,season, plot, square) %>%
  ...
  squares <- habitat %>% 
    ...
    distinct() %>% 
    left_join(trees, by=c("grid_name","season","plot","square"))
  ...
  plot_sum <- squares %>% 
    ...
    group_by(pitfall, pitfall_line, grid_name, season, plot) %>% 
  ...
  grid_sum_mat <- grid_sum %>% 
    unite(col="grid_season", grid_name:season, remove = TRUE) %>%
